...
def show_results(img, results, img_width, img_height, model_age, model_gender):
	...
	for i in range(len(results)):
		...
		# lines_age=open('./agegender/words/agegender_age_words.txt').readlines()
		# lines_gender=open('./agegender/words/agegender_gender_words.txt').readlines()
		# lines_fer2013=open('./agegender/words/emotion_words.txt').readlines()
		...
		if(model_gender!=None):
			...
			gender_labels.append(cls_gender_keras)
	...
	return img_cp, age_labels, gender_labels
	
def detect_face_age_gender(img_ori, model_face, model_age, model_gender, frame):
	...
	face_results = interpret_output_yolov2(out2, img.shape[1], img.shape[0])

	# Do nothing just speed up when the first face occur. To avoid buffer
	if (frame == 0):
		shape = model_age.layers[0].get_output_at(0).get_shape().as_list()
		img_keras = cv2.resize(img_cv, (shape[1],shape[2]))
		img_keras = np.expand_dims(img_keras, axis=0)
		img_keras = img_keras / 255.0
		_ = model_age.predict(img_keras)[0]
		_ = model_gender.predict(img_keras)[0]

	#Age and Gender Detection
	img_cp, age_labels, gender_labels = show_results(img_cv, face_results, img.shape[1], img.shape[0], model_age, model_gender)
	
	return face_results, age_labels, gender_labels

def match_face(face_boxes, people_boxes):
	"""
	face_boxes: shape [M, 4]
	people_boxes: shape [N, 4]
	"""
	people_boxes = np.expand_dims(people_boxes, axis=0)
	face_boxes = np.expand_dims(face_boxes, axis=1)

	people_xymin = people_boxes[:, :, 0:2]
	people_xymax = people_boxes[:, :, 2:4]

	face_xymin = face_boxes[:, :, 0:2]
	face_xymax = face_boxes[:, :, 2:4]

	intersect_mins = np.maximum(people_xymin, face_xymin)
	intersect_maxs = np.minimum(people_xymax, face_xymax)
	intersect_wh = np.maximum(intersect_maxs - intersect_mins, 0.0)

	intersect_area = intersect_wh[:, :, 0] * intersect_wh[:, :, 1]

	face_wh = face_xymax - face_xymin
	face_area = face_wh[:, :, 0] * face_wh[:, :, 1]

	ratio = intersect_area / (face_area + 1e-10)

	# Shape [M, N]
	return ratio

def match_people(now_boxes, last_boxes):
	"""
	Now_boxes: shape [M, 4]
	Last_boxes: shape [N, 4]
	"""
	last_boxes = np.expand_dims(last_boxes, axis=0)
	now_boxes = np.expand_dims(now_boxes, axis=1)

	last_xymin = last_boxes[:, :, 0:2]
	last_xymax = last_boxes[:, :, 2:4]

	now_xymin = now_boxes[:, :, 0:2]
	now_xymax = now_boxes[:, :, 2:4]

	intersect_mins = np.maximum(last_xymin, now_xymin)
	intersect_maxs = np.minimum(last_xymax, now_xymax)
	intersect_wh = np.maximum(intersect_maxs - intersect_mins, 0.0)

	intersect_area = intersect_wh[:, :, 0] * intersect_wh[:, :, 1]

	now_wh = now_xymax - now_xymin
	now_area = now_wh[:, :, 0] * now_wh[:, :, 1]

	last_wh = last_xymax - last_xymin
	last_area = last_wh[:, :, 0] * last_wh[:, :, 1]

	iou = intersect_area / (last_area + now_area - intersect_area + 1e-10)
